...
この解析結果を相手に Web アプリで対局するには,

* ターミナル上で `make RULES=val1n -C precomp` を実行

続く手順は後述. 容量を節約したけば下記でもよい.

* ターミナル上で `make RULES=val1n small -C precomp` を実行

アプリの容量が, たとえば 160MB から 5MB に減る. ただし, 完全解析の結果が後手勝ちでなければならず, また後手は決まった手しか指さない.
