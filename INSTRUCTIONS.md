...
* ターミナル上で `make -C precomp RULES=val1n analyze` を実行
...
* `init depth:   0` → 双方最善に指せば引分 (ループまたは「合法手のない局面」に至る)
...
※ 隠し機能: `RULES=val1nT` のように末尾に「T」をつければ, 「トライ」のルールを無効化.
...
* ターミナルで `ruby -run -e httpd docs -b 127.0.0.1 -p 8000` を実行 (止めるには `Ctrl-C` キー)
* ブラウザで <http://localhost:8000/> を開く

解析結果ファイルは `precomp/` ディレクトリに生成され, その中から Web アプリに必要なファイルが `docs/` にコピーされる. 作業後は, 不要なら前者ファイルは消してもよい.

* ターミナルで `make -C precomp clean` →  `precomp/` ディレクトリの中間生成ファイルを消す
* `make -C precomp distclean` → `precomp/` ディレクトリの解析結果ファイルをすべて消す (中間物も最終結果も)

※ 容量節約版も当初は用意したが, その後メンテしていないので不具合があるかもしれない (アプリの容量が, たとえば 50MB 超から 5MB に減る. ただし, 完全解析の結果が後手勝ちでなければならず, また後手は決まった手しか指さない. 手番交代などの拡張機能も使えない). 試すには, 上記 `make -C precomp RULES=val1n` のかわりに `make -C precomp RULES=val1n smalldocs` を実行.
...
* 深さ 4: あと 4 手で負ける
* ……
* 深さ -1: 双方最善に指せば引分 (ループまたは「合法手のない局面」に至る)
...
#### `keys.xz`, `vals.xz` (各数十 MB)

`2_cooked.txt` の局面文字列のみを抽出したのが `keys.xz`, 深さのみを抽出したのが `vals.xz`. 容量節約のため, どちらもバイナリファイルに詰めてさらに xz 圧縮している. 「AI」はこれを参照して指す.

* `keys.xz`: 局面文字列 (16 進数 15 桁) を 64bit 符号なし整数 (リトルエンディアン) としてパック
* `vals.xz`: 16bit 符号なし整数 (リトルエンディアン) としてパック

ただし, keys のほうは実際は, 前行との差分を記録している. そのほうが圧縮が効きやすい (たとえば 35MB → 21MB).

また, vals は当初 8bit だったが, ルールによっては深さが 256 以上になるため, 16bit に拡張した. 圧縮すればファイルサイズは大差ない (たとえば 34MB → 35MB). 対局 Web アプリはどちらにも対応.

#### `3.txt` (数百万行, 数十MB)

※ ここからは, 後手必勝という前提でさらにファイルサイズを絞った版. `make ... smalldocs` の場合は, `keys.xz`, `vals.xz` のかわりに以下が使われる.
...
※ 実装上, 左右反転した盤面でも同じ文字列が出力される. `.../encode.rb -v` なら, 入力盤面と左右反転盤面に対応する文字列がそれぞれ別に出力される.
...
* 残り 12 文字「00a09001b002」 … 盤面 (48bit)
  * 00a0 → 盤面左列は上から順に, 空・空・相手象・空
  * 9001 → 盤面中列は上から順に, 相手ライオン・空・空・手番ライオン
  * b002 → 盤面右列は上から順に, 相手きりん・空・空・手番象
  * 文字は
    * 0 空
    * 1 手番ライオン
    * 2 手番象
